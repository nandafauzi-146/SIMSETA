...
<div class="mx-auto max-w-5xl"
    x-data="{
        kategori: '{{ old('kategori', $default_kategori ?? 'masyarakat') }}',
        statusTanah: '{{ old('status_nama', 'Aktif') }}'
    }">

    <x-admin.page-header title="Tambah Data Tanah"
        description="Lengkapi detail formulir di bawah ini untuk menambahkan aset tanah baru ke dalam sistem." />

    <x-admin.form-card>
        <form method="POST" action="{{ route('admin.sertifikat.store') }}" class="space-y-8">
            @csrf
            ...
            <div>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-[var(--text)]">NIB (Nomor Identifikasi Bidang)</label>
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_hak_nama" required placeholder="Contoh: Hak Milik, Hak Pakai, Letter C"
                            value="{{ old('jenis_hak_nama', 'Hak Milik') }}"
                ...
                <div x-show="kategori === 'masyarakat'" x-transition class="grid gap-5 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat_pemilik" rows="2" placeholder="Alamat domisili pemilik saat ini..."
                            class="w-full rounded-2xl border border-slate-200 bg-[var(--bg)]/50 px-4 py-3 text-sm focus:border-[var(--primary)] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[var(--primary)]/10 transition">{{ old('alamat_pemilik') }}</textarea>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-[var(--text)]">Dukuh <span class="text-red-500">*</span></label>
                        <select name="desa_id" required
                            ...
                            @foreach ($desas as $d)
                                <option value="{{ $d->id }}" {{ old('desa_id') == $d->id ? 'selected' : '' }}>{{ $d->dusun ?: $d->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" rows="2" placeholder="Alamat lengkap lokasi tanah..."
                            class="w-full rounded-2xl border border-slate-200 bg-[var(--bg)]/50 px-4 py-3 text-sm focus:border-[var(--primary)] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[var(--primary)]/10 transition">{{ old('alamat') }}</textarea>
                    </div>
                    <div>
