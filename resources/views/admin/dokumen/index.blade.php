...
@section('content')
    <div class="mx-auto max-w-4xl">
        <x-admin.page-header title="Dokumen: {{ $sertifikat->nomor_sertifikat }}"
        ...
        <x-admin.form-card>
            <form method="POST" action="{{ route('admin.sertifikat.dokumen.store', $sertifikat) }}"
                            ...
                            <div class="flex gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.sertifikat.dokumen.destroy', [$sertifikat, $d]) }}"
                                    ...
                                    <div x-show="open" x-cloak
                                        ...
                                        @click="open = false">
                                        <div class="mx-4 w-full max-w-xs animate-modal rounded-2xl bg-white p-6 shadow-2xl"
                                            @click.stop>
                                            <div class="flex flex-col items-center text-center">
                                                <div class="flex h-12 w-12 items-center justify-center rounded-full bg-red-50 mb-3">
                                                    <i class="fas fa-trash text-lg text-red-500"></i>
                                                </div>
                                                <h3 class="text-base font-bold text-[var(--text)]">Hapus Dokumen</h3>
                                                <p class="mt-1 text-sm text-[var(--text-muted)]">Dokumen <strong class="text-[var(--text)]">{{ $d->nama_file }}</strong> akan dihapus permanen.</p>
                                            </div>
                                            <div class="mt-5 flex gap-2">
                                                <button type="button" @click="open = false"
                                                    class="flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-[var(--text)] transition hover:bg-slate-50">
                                                    Batal
                                                </button>
                                                <button type="submit"
                                                    class="flex-1 rounded-xl bg-red-600 px-3 py-2.5 text-sm font-semibold text-white transition hover:bg-red-700">
